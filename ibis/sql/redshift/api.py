...
def compile(expr):
    ...
    """
    from .client import RedshiftDialect
    from ibis.sql.alchemy import to_sqlalchemy
    return to_sqlalchemy(expr, dialect=RedshiftDialect)


def connect(host=None, user=None, password=None, port=None, database=None,
            url=None, driver=None, schema=None):
    ...
    """
    return RedshiftClient(host=host, user=user, password=password, port=port,
                          database=database, url=url, driver=driver, schema=schema)
